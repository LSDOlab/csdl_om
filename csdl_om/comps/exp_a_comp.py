...
class ExpAComp(ExplicitComponent):
    '''
    ...
    def compute(self, inputs, outputs):
        ...
        out_name = self.options['out_name']
        a = self.options['a']
        outputs[out_name] = a**inputs[in_name]

    def compute_partials(self, inputs, partials):
        in_name = self.options['in_name']
        out_name = self.options['out_name']
        a = self.options['a']
        partials[out_name, in_name] = (a**inputs[in_name] * np.log(a)).flatten()
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        'exp',
        ExpAComp(a=5.0, in_name='x', out_name='y', shape=(n, ), val=val),
        promotes=['*'],
    )
    prob.setup()
